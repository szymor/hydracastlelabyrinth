...
static uint64_t tframe;

float ratio = 1.3333333; // Aspect ratio for 320x240, for SDL3 HW scaling.
...
void PHL_GraphicsInit()
{
	...
	window = SDL_CreateWindow("HCL", screenW, screenH, flags);
	if(!window)
	{
	...
	// match SDL2 behavior: SDL3 defaults to LINEAR filtering, which blurs sprites
	SDL_SetDefaultTextureScaleMode(renderer, SDL_SCALEMODE_NEAREST);

	if (!xbrz) {
		// Scale using SDL3 renderer, NOT in software.
...
void PHL_EndDrawing()
{
	...
	static int skip = 0;
	uint64_t tnext = tframe + 1000/FPS;
	...
	SDL_FRect rect = {deltaX, deltaY, actualW, actualH};
...
{
    SDL_SetSurfaceColorKey(surf.surf, true, SDL_MapSurfaceRGB(surf.surf, r, g, b));
	if(surf.tex)
...
{
	PHL_Surface ret = {0};
	// Same layout as the old SDL2 masks (0x00ff0000,0x0000ff00,0x000000ff,0xff000000):
	// QDA loader writes 0xAARRGGBB pixels, which SDL3 interprets as ARGB8888.
	ret.surf = SDL_CreateSurface(w, h, SDL_PIXELFORMAT_ARGB8888);
	ret.tex = NULL;
	return ret;
}
...
{
	// use integer coordinates, like SDL_Rect in the SDL2 backend
	SDL_FRect rect = {x*screenScale/2, y*screenScale/2, w*screenScale/2, h*screenScale/2};
...
void PHL_DrawSurface(double x, double y, PHL_Surface surface)
{
	...
	SDL_FRect offset;
	// snap to integers, matching SDL_Rect behavior in the SDL2 backend
	offset.x = (int)(x*screenScale/2);
	offset.y = (int)(y*screenScale/2);
	offset.w = surface.surf->w;
	offset.h = surface.surf->h;
	
	SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
	SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
	SDL_RenderTexture(renderer, surface.tex, NULL, &offset);
}
void PHL_DrawSurfacePart(double x, double y, int cropx, int cropy, int cropw, int croph, PHL_Surface surface)
{
	...
	SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
	SDL_RenderTexture(renderer, surface.tex, &crop, &offset);
}
